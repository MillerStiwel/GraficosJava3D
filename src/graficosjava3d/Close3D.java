...
public final class Close3D extends Applet {
    private static final long serialVersionUID = 1L;
    
    public static void main(String[] args){
        if (args.length>=1)
            new MainFrame(new Close3D(args[0]),1280,720);
        else
            new MainFrame(new Close3D(null),1280,720);
    }
    
    public Close3D (String filename){
        //se crea el universo
    ...
    public BranchGroup crearescenaG(String filename){
        ...
        try{
           s = file.load(filename == null ? "src\\images\\wardrobe_OBJ.obj" : filename);
